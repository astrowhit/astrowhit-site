...
- **Publications list is up to date as of August 2026.** `publications.html` includes 230 refereed publications plus 8 submitted/in-review works, each linked to ADS, organized by year back to 2005. Papers that moved from "Submitted Works" to a refereed journal since the original build were checked individually against arXiv/ADS and re-filed into 2025/2026 sections with updated journal citations; one new Whitaker co-authored preprint (Cloonan et al. 2026, arXiv:2603.24700) was also found via search and added. The h-index/citation totals in the header line are still the November 2025 snapshot — a live ADS API pull would be needed to refresh those two numbers specifically.
- **News: all 32 posts are mirrored locally**, including the two oldest (2017–2018) that were originally missing.
- **Four previously-missing standalone pages are now mirrored locally**: `astronomy-research-tutorial-repository.html`, `data-products.html`, `graduate-student-research-agreement.html`, and `data-challenge.html` (none of these have their own nav-bar tab, matching the original site; they're linked from Research, Teaching, Outreach, and Mentoring Resources same as before). The *pages* no longer depend on astrowhit.com, but each links out to a number of individual downloadable resource files (PDFs, Jupyter notebooks, data files) that are still hosted at `astrowhit.com/s/...` — the build sandbox used to create this site blocks outbound downloads to that domain, so those specific files could not be fetched and rehosted. If astrowhit.com goes offline, those individual download links will break even though the pages themselves will keep working. The same is true of the two Eureka Workshop worksheet PDFs linked from `outreach.html`.
